...
prepare.data<- function(trees, data, scoring, reference_table) {
  ...
  for(i in 1:length(trees)) {
    ...
    write.csv(result_traits, file=paste0("trait_datasets/",one_label,"_life_form.csv"), row.names = F)
...
# (3) organize data.frame to include also tree name (OK)


# Tree plots
pdf(paste0(getwd(), "/figures/life_form_overview.pdf"), width= 4, height= 12)
