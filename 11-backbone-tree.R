...
clade_trees_files <- list.files("trees_simplified_tips", full.names=T)
labels <- gsub(paste0(c("trees_simplified_tips/","_cleaned.tre"), collapse="|"),"", clade_trees_files)
clade_trees <- lapply(clade_trees_files, ape::read.tree)
clade_tip_labels <- lapply(clade_trees, function(x) fix.names.taxize(x$tip.label))
names(clade_tip_labels) <- labels

tips_to_keep <- c()
for(i in 1:length(clade_tip_labels)){
  one_label <- names(clade_tip_labels)[i]
  one_tree_tip_labels <- clade_tip_labels[[i]]
  one_tree_tip_labels <- gsub("_"," ",one_tree_tip_labels)
...
# Adding Heliophileae-Huang_et_al-2019 as sister to Brassiceae and Thelypodieae

node0 <- fastMRCA(pruned_big_tree, pruned_big_tree$tip.label[grep("Brassiceae", pruned_big_tree$tip.label)] , pruned_big_tree$tip.label[grep("Thelypodieae", pruned_big_tree$tip.label)]
)
node <- Ancestors(pruned_big_tree, node0, type="parent")

# getting position and edge length for new tip 
initial_length <- get.node.age(pruned_big_tree)[node]
desc_node <- subset(Children(pruned_big_tree, node), !Children(pruned_big_tree, node) %in% 1:length(pruned_big_tree$tip.label)
)
final_length <- get.node.age(pruned_big_tree)[desc_node]
edge_length_input <- (initial_length + final_length) / 2
position_input <- (initial_length - final_length) / 2

backbone_tree <- bind.tip(pruned_big_tree, tip.label="Heliophileae-Huang_et_al-2019",where=node0, position=position_input, edge.length=edge_length_input)

backbone_tree <- force.ultrametric(backbone_tree)
